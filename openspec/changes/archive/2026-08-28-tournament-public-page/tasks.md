...
- [x] 3.2 Page layout: `parentEvent` set → `h1` event title (fallback `event.slug`), `EventTournamentTabs`, `h2` tournament title; otherwise `h1` tournament title; then `TournamentMeta`, `tabs tabs-box` tablist (Описание/Расписание/Игроки/Результаты/Таблица with `BsJournalText`/`BsClock`/`HiOutlineUserGroup`/`Bs123`/`BsGrid3X2`), conditional sections («Описание» and «Результаты» empty placeholders).
