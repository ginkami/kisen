...
The admin drawer's "Events" section SHALL render a month selector (with `BsCalendar2` icon), a role-gated "+ Мероприятие" create button, and a scrollable list of events available to the user for the selected month. The section SHALL be available to users with role `admin` or `manager`, and to invited managers who have events under their associations.
