...
- [x] 1.1 Create `src/components/tournament/PairingToolsDrawer.tsx`: right-side fixed drawer (`w-80`, `bg-base-200`, `translate-x-full` when closed), sticky header with `FaPeopleArrows` + `tournament.edit.pairingTools.title`, `BsX` close button, Escape-to-close, empty content body
...
- [x] 3.3 `PairingsBoard`: optional `onTogglePairingTools`; render the `FaPeopleArrows` button in the rows header slot (`☗ [FaPeopleArrows] ☖`) only when the callback is provided
- [x] 3.4 `TournamentEditForm`: render the right-side sticky tab (`fixed right-0 top-17 rounded-l-box`, `FaPeopleArrows`) when available and the drawer is closed, and the drawer when open; pass `pairingToolsAvailable` / `onTogglePairingTools` to `PairingsSection`
...
- [x] 5.2 Extend `src/test/pairingsBoard.test.tsx`: header dice (FaPeopleArrows) button renders with the callback and invokes it; absent without it
