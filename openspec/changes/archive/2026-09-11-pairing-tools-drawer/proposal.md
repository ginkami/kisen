...
- New `PairingToolsDrawer` component: a right-side drawer (mirroring the AdminDrawer chrome, overlaying without pushing content) with the title "Подобрать пары" / "Pairing assistant" (`FaPeopleArrows` icon) and empty content for now.
...
- Two toggle points (both `FaPeopleArrows`): a sticky right-edge tab (mirroring the admin drawer's left tab) and a button in the pairings rows header (`☗ [FaPeopleArrows] ☖`) between the player-side headers.
...
- **Affected code:**
  ...
  - `src/components/tournament/PairingsBoard.tsx` — optional `onTogglePairingTools` prop rendering the dice (FaPeopleArrows) button in the rows header
