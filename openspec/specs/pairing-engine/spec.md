...
Pure Swiss pairing engine: automatic pairing of the round being prepared (publishedRounds + 1) via a weighted graph of candidate pairs and the Edmonds blossom maximum-weight matching algorithm, with exponential score-group dominance, rating subgroup emulation inside score groups, bye assignment through a virtual node, hard constraint enforcement (rematches, double skips, color safety) and deterministic sente assignment.
