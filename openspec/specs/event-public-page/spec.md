...
Public event page served at `/events/{UUID}` and `/events/{slug}`: single route with UUID param sniffing, event header and page states, in-place rendering of the latest public tournament, and an event description + regulations branch when there are no public tournaments.
