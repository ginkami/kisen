...
Client-side gating of event edit access: a single shared editability rule mirroring the Firestore event-update rules, the service-level editable events listing, the admin drawer's event availability and search filtering, the tournament event picker restriction, and the direct-URL guard on the event edit page.
