...
The home page lists published tournaments (isPublic with status finished, ongoing, or upcoming) in three phase tabs with counts, ordered by start time and loaded in pages of 30 via cursor pagination, with a filter form (title, date range, country, city) and a tournament card linking to the tournament, event, and edit pages.
